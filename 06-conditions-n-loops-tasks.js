...
function doRectanglesOverlap(rect1, rect2) {
    // cosnt {top, left, width, height} = rect1;
    // cosnt {top: top2, left: left2, width: width2, height: height2} = rect2;

    // return ((left + width >= left2) &&
    //     (left <= left2 + width2) &&
    //     ((top + height >= top2) &&
    //         (top <= top2 + height2)));
    ...
    if (arr.length % 2 === 0) {
        for (let i = 0; i < arr.length; i = i + 2) {
            if (Number(arr[i]) * 2 > 9) {
                ...
                sum = sum + (arr[i] * 2 - 9) + Number(arr[i + 1]);
            }
            else {
                sum = sum + (arr[i] * 2) + Number(arr[i + 1]);
            }
        }
        if (sum % 10 === 0) {
            return true
        ...
        for (let j = 1; j < arr.length; j = j + 2) {
            if (arr[j] * 2 > 9) {
                ...
                sum = sum + (arr[j] * 2 - 9) + Number(arr[j + 1]);
            }
            else {
                sum = sum + (arr[j] * 2) + Number(arr[j + 1]);
            }
        }
        if ((Number(arr[0]) + sum) % 10 === 0) {
            return true
        }
    }
    return false
...
function timespanToHumanString(startDate, endDate) {
    let startDate1 = Date.parse(startDate) / 1000;
    let endDate1 = Date.parse(endDate) / 1000;
    let differenceInSeconds = endDate1 - startDate1;
    let differenceInMinutes = differenceInSeconds / 60;
    let differenceInHours = differenceInMinutes / 60;
    let differenceInDays = differenceInMinutes / 24;
    if (differenceInSeconds <= 45) {
        return 'a few seconds ago';
    }
    if (45 < differenceInSeconds <= 90) {
        return 'a minute ago';
    }
    if (differenceInSeconds > 90 && differenceInMinutes <= 45) {
        return differenceInMinutes + 'minutes ago';
    }
    if (45 < differenceInMinutes <= 90) {
        return 'an hour ago';
    }
    if (differenceInMinutes > 90 && differenceInHours <= 22) {
        return differenceInHours + 'hours ago';
    }
    if (22 < differenceInHours <= 36) {
        return 'a day ago';
    }
    if (differenceInDays <= 25 && differenceInHours > 36) {
        return differenceInDays + 'days ago';
    }
    if (25 < differenceInDays <= 45) {
        return 'a month ago'
    }
    if (45 < differenceInDays <= 365) {
        return differenceInDays / 30 + 'months ago';
    }
    if (345 < differenceInDays <= 545) {
        return 'a year ago';
    }
    if (differenceInDays > 545) {
        return differenceInDays / 365 + 'years ago';
    }
}
...
function getCommonDirectoryPath(pathes) {
    ...
    for (let i = 0; i < pathes[0].length; i++) {
        let char = pathes[0][i];
        for (let j = 1; j < pathes.length;) {
            if (char === pathes[j][i]) {
                j++
            } else if (char !== pathes[j][i]) {
                let arr = result.split('').reverse();
                if (arr[0] === '/') {
                    return result;
                } else {
                    let index = arr.indexOf('/');
                    let lengthOfResult = result.length - index;
                    return result.substring(0, lengthOfResult);
                }
            }
        }
        result = result + char;
